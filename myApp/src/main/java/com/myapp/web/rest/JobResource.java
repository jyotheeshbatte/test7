...
public class JobResource {
     ...
     */
    @GetMapping("/jobs")
    public Mono<List<Job>> getAllJobs() {
        log.debug("REST request to get all Jobs");
        return jobService.findAll().collectList();
    }

    /**
     * {@code GET  /jobs} : get all the jobs as a stream.
     * @return the {@link Flux} of jobs.
     */
    @GetMapping(value = "/jobs", produces = MediaType.APPLICATION_NDJSON_VALUE)
    public Flux<Job> getAllJobsAsStream() {
        log.debug("REST request to get all Jobs as a stream");
